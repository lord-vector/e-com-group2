...
const showItems = (items) => {
  ...
  items.forEach((element) => {
    str += `<div class="card" style="width: 18rem;">
      <img src="${element.images[0]}" class="card-img-top imageShadow" alt="..." />
      <div class="card-body">
    ...
    .then((res) => {
      ...
      res.json().then((data) => {
        ...
        let finalPrice = Math.round(
          temp[i].price - (temp[i].price * temp[i].discountPercentage) / 100
        );
...
const closeCart = () => {
  document.getElementById("cart").style.right = "-600px";
  document.getElementById("cart").style.boxShadow = "none";
};

const hooray = () => {
  document.getElementsByClassName("cart-items")[0].innerHTML =
    "Order Successfully placed!<br>Thank you for shopping with us :)";
};
